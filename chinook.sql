...
select  invoice.BillingCountry, sum(invoice.Total) from invoice group by invoice.BillingCountry having sum(invoice.Total)>100 order by sum(invoice.Total) desc ;
...
select distinct employee.Phone as phonenumber from employee left join customer
on employee.EmployeeId = customer.SupportRepId inner join invoice on customer.CustomerId = invoice.CustomerId
inner join invoiceline i on invoice.InvoiceId = i.InvoiceId
inner join track t on i.TrackId = t.TrackId
inner join mediatype m on t.MediaTypeId = m.MediaTypeId where Composer like '%Miles Davis%'and m.Name='MPEG Audio File';
...
select album.AlbumId, album.Title from album left join
track t on album.AlbumId = t.AlbumId inner join
genre g on t.GenreId = g.GenreId where g.GenreId='11' and album.Title like 'samba%';
...
select genre.Name, AVG(Milliseconds/60000) AS minutes  from genre left join track on genre.genreid=track.genreid where Milliseconds > 1800000 group by  genre.Name order by minutes desc;
